...
export const dates = (() => {
    ...
    const translateMonths = [{1: 'Janvier'}, {2: 'Février'}, {3: 'Mars'}, {4: 'Avril'}, 
    {5: 'Mai'}, {6: 'Juin'}, {7: 'Juillet'}, {8: 'Août'}, {9: 'Septembre'}, 
    {10: 'Octobre'}, {11: 'Novembre'}, {12: 'Décembre'}, ]
    
    const getDates = (date) => {
        const day = translateDays.find(element => element[format(new Date(date), 'e')] != null); 
        const dayOfTheWeek = Object.values(day)[0];
        const dayOfTheMonth = format(new Date(date), 'd');
        const month = translateMonths.find(element => element[format(new Date(date), 'M')] != null);
